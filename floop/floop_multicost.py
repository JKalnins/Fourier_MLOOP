...
def _SquareDiff(guess, y_target):
    """Calculates the true cost (mean squared difference) between an FS guess and a target
    ...
    Returns:
        float: mean-square-difference cost
    """
    n_ab = len(guess)
    a_g = guess[:n_ab]
    b_g = guess[n_ab:]
...
def Cost(guess, y_targets, heights, noise_type="None", noise_scale=0.0):
    """Calculates the cost of an FS guess against a target, including noise

    Args:
        guess (np.ndarray): a,b-parameters for the guess
        y_targets (list): list of np.ndarrays with y-values for each target
        heights (list): list of height factors
    ...
    # cost = product(cost_i + alpha)
    square_diff = 1
    for (y_target, height) in zip(y_targets, heights):
        square_diff_i = _SquareDiff(guess, y_target)
        square_diff *= square_diff_i + height
...
class _FourierInterface(mli.Interface):
    ...
    def get_next_cost_dict(self, params_dict):
        ...
        cost, uncert = Cost(
            guess, self.y_targets, self.heights, self.noise_type, self.noise_scale
        )
...
def RunOnce(
    ...
    """
    interface = _FourierInterface(n_ab, y_targets, heights, noise_scale, noise_type)
    ...
    # controller.optimize is the actual "run" of the controller
    controller.optimize
...
def _SaveNPZ(filename, *objs):
    ...
    if not os.path.isdir("./npz"):
        os.mkdir("./npz")
    np.savez(f"./npz/multi_{filename}.npz", *objs)
...
def RepeatRuns(
    ...
    Args:
        ...
        num_in_targets (int, optional): Number of func's to be combined to create target function. Defaults to 2.
        y_targets_reps (list, optional): If specified, gives a target function. If not specified, random FS is used each time. Defaults to None.
        heights (list, optional): If specified, defines the height of each cost function. Defaults to [0, 1e-3].
        noise_type (str, optional): Type of noise ("None", "add", or "multi"). Defaults to "None".
        noise_scale (float, optional): Std Dev of noise if noise_type != "None". Defaults to 0.0.
        sleep_time (float, optional): time in seconds between runs in case each run needs a separate time-label

    Raises:
        ValueError: Raised if the length of heights is not num_in_targets
        ValueError: Raised if the shape of y_targets_reps is repeats x num_in_targets
    ...
    min_costs_list = []
    start_times = []
    rep_lengths = np.zeros(repeats)

    # create y_targets_reps if needed
    if not y_targets_reps:
        y_targets_reps = [[] for _ in range(repeats)]
        for rep in range(repeats):
            for _ in range(num_in_targets):
                a_t = (np.random.random(n_ab) * 2) - 1
                b_t = (np.random.random(n_ab) * 2) - 1
                y_targets_reps[rep].append(FourierFromParams(a_t, b_t))

    # Checking shapes
    if not num_in_targets == len(heights):
        raise ValueError("heights must have num_in_targets elements")
    if not (
        len(y_targets_reps) == repeats and len(y_targets_reps[0]) == num_in_targets
    ):
        raise ValueError("Provided y_targets_reps list was not the correct shape.")

    # repeatedly run M-LOOP
    for rep in range(repeats):
        y_targets = y_targets_reps[rep]
        start_time = datetime.datetime.now()
        start_times.append(_TimeToString(start_time))
        # ------------------------------------------------#
        costs, runs = RunOnce(
            max_allowed_runs, tcost, n_ab, y_targets, heights, noise_type, noise_scale
        )
        # ------------------------------------------------#
        costs_list.append(costs)
        min_costs_list.append(_MinCosts(costs))
        rep_lengths[rep] = runs
        if sleep_time > 0.0:
            if rep != repeats - 1:
                time.sleep(sleep_time)  # so it doesn't use the same timestamp twice

    # equalise costs lists if required
    if np.ndarray.all(rep_lengths == rep_lengths[0]):
        max_runs = rep_lengths[0]
        costs_arr = np.array(costs_list)
        min_costs_arr = np.array(min_costs_list)
    else:
        costs_arr, max_runs = _CostLengthEqualiser(costs_list, rep_lengths, repeats)
        min_costs_arr, _ = _CostLengthEqualiser(min_costs_list, rep_lengths, repeats)
...
def ReadRepeatNPZ(filename):
    ...
    n_ab = int(npz["arr_3"])
    y_targets_reps = npz["arr_4"]
    heights = list(npz["arr_5"])
